...
class Vista_reporte_compensacion(QWidget):
    ...
    def __init__(self, interfaz, actividad):
        """
    ...
    def mostrar_reporte_compensacion(self, actividad):
        ...
        """        
        matriz_compensacion = Actividad.calcular_matriz(actividad)
            ...
            for j in range(0, len(self.matriz)):
                ...
                if i == j:
                    ...
                    etiqueta_valor.setToolTip(
                        self.matriz[0][j] + " debe " + "${:,.2f}".format(self.matriz[i][j]) + " a " + self.matriz[0][i])
