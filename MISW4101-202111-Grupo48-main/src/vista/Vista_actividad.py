...
class Vista_actividad(QWidget):
    ...
    def __init__(self,principal, actividad):
        """
        ...
        self.width = 720
        self.height = 550
        self.inicializar_GUI(actividad)
        self.show()
       

    def inicializar_GUI(self, actividad):
        ...
        self.btn_aniadir_gasto.clicked.connect(partial(self.crear_gasto,actividad))
        ...
        self.btn_reporte_gastos_viajeros.clicked.connect(partial(self.mostrar_reporte_gastos_por_viajero, actividad))
        ...
        self.btn_reporte_compensacion.clicked.connect(partial(self.mostrar_reporte_comensacion, actividad))
    ...
    def mostrar_gastos_por_actividad(self, actividad):
        ...
        self.gastos = Ver_actividad.get_gastos(actividad)
        ...
        for gasto in self.gastos:
            ...
            etiqueta_nombre = QLabel(Ver_actividad.get_viajero(gasto))
            ...
            btn_editar.clicked.connect(partial(self.editar_gasto, gasto,actividad))
    ...
    def crear_gasto(self,actividad,viajero):
        """
        Esta función ejecuta el diálogo para crear un gasto
        """    
        dialogo = Dialogo_crear_gasto(self.interfaz.dar_viajeros(),None,actividad,viajero)
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.insertar_gasto(actividad,dialogo.concepto.text(), str(dialogo.fecha.date().toPyDate().strftime("%d-%m-%Y")), dialogo.valor.text(), dialogo.lista_viajeros.currentText().split(" ")[0], dialogo.lista_viajeros.currentText().split(" ")[-1])
        else:
            self.mostrar_gastos_por_actividad(actividad)

    def editar_gasto(self, gasto,actividad):
        """
        Esta función ejecuta el diálogo para editar un gasto
        """
        viajero=Crear_gasto.poblar_viajero(gasto.id,actividad)
        dialogo = Dialogo_crear_gasto(self, gasto,actividad,viajero)
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.insertar_gasto(actividad,dialogo.concepto.text(), str(dialogo.fecha.date().toPyDate().strftime("%d-%m-%Y")), dialogo.valor.text(), dialogo.lista_viajeros.currentText().split(" ")[0], dialogo.lista_viajeros.currentText().split(" ")[-1])
        else:
            self.interfaz.editar_gasto(actividad)
