...
class Dialogo_crear_gasto(QDialog):
    ...
    def __init__(self, viajeros, gasto, actividad, viajero):
        ...
        self.btn_guardar.clicked.connect(
            partial(self.guardar, actividad, self.concepto, self.fecha, self.valor, self.lista_viajeros))
        ...
        if gasto != None:
            ...
            distribuidor_dialogo.addWidget(self.btn_guardar, numero_fila, 1)
            self.btn_guardar.clicked.connect(
                partial(self.guardar_editado, actividad, gasto, self.concepto, self.fecha, self.valor,
                        self.lista_viajeros))
    ...
    def guardar(self, actividad, concepto, fecha, valor, viajeros):
        ...
        """
        resultado = Crear_gasto.crear_gasto_listo(actividad, concepto.text(), fecha.date(), valor.text(),
                                                  viajeros.currentText())
        if resultado == False:
            resultado = 0
        else:
            resultado = 1
    ...
    def guardar_editado(self, actividad, gasto, concepto, fecha, valor, viajeros):
        ...
        Esta función envía la información de que se han guardado los cambios
        """
        Crear_gasto.editar_gasto_listo(actividad, gasto, concepto.text(), fecha.date(), valor.text(),
                                       viajeros.currentText())
        self.close()
